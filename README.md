<!--
README for GitHub profile repo:
Create a repo named exactly: YOUR_GITHUB_USERNAME
Then add this as README.md
-->

<div align="center">

# Etay Teichman

**B.Sc. Computer Science & Linguistics @ Ben-Gurion University (2024–2027)**  
Data Analysis • ML Foundations • NLP

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=18&pause=1200&center=true&vCenter=true&width=700&lines=Data+Analysis+%7C+NLP+%7C+Machine+Learning+Foundations;Python+%7C+SQL+%7C+Java;Building+data+systems+that+drive+decisions" alt="Typing SVG" />

---

### Quick Links
<a href="https://www.linkedin.com/in/etay-teichman/">LinkedIn</a> •
<a href="mailto:etayteichman@gmail.com">Email</a>

</div>

---

## About Me
Second-year B.Sc. student in **Computer Science and Linguistics** with a strong focus on **data analysis**, **machine learning foundations**, and **NLP**.  
I enjoy turning complex datasets into actionable insights and building practical data systems to solve real-world problems.

---

## Skills

### Programming
![Python](https://img.shields.io/badge/Python-000000?logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-000000?logo=postgresql&logoColor=white)
![Java](https://img.shields.io/badge/Java-000000?logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-000000?logo=javascript&logoColor=white)
![C++](https://img.shields.io/badge/C%2B%2B-000000?logo=c%2B%2B&logoColor=white)
![C#](https://img.shields.io/badge/C%23-000000?logo=csharp&logoColor=white)

### Tools & Technologies
![Git](https://img.shields.io/badge/Git-000000?logo=git&logoColor=white)
![Excel](https://img.shields.io/badge/Excel-000000?logo=microsoftexcel&logoColor=white)
![Microsoft](https://img.shields.io/badge/Microsoft-000000?logo=microsoft&logoColor=white)

### Strengths
- Strong team collaboration  
- High independence & self-learning  
- Time management  
- Project management mindset  


---

## Education
**Ben-Gurion University of the Negev** — B.Sc. Computer Science & Linguistics (2024–2027)

---

## Featured Projects
> Pin your best repos on GitHub and mirror them here.

- **Project Name 1** — one-liner impact (SQL / Python / NLP)  
  Repo: https://github.com/YOUR_GITHUB_USERNAME/REPO_NAME  
- **Project Name 2** — one-liner impact (ML foundations / data pipelines)  
  Repo: https://github.com/YOUR_GITHUB_USERNAME/REPO_NAME  
- **Project Name 3** — one-liner impact (dashboards / analytics)  
  Repo: https://github.com/YOUR_GITHUB_USERNAME/REPO_NAME  

---



## Contact
📩 **etayteichman@gmail.com**

<!-- Optional:
- Add a "Currently learning" section
- Add a "Certificates" section
- Add a "Resume" PDF link once hosted (Drive/website)
-->
